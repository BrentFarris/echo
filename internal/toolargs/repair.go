...
// RepairJSON returns a valid JSON argument payload when a small deterministic
// repair can recover a truncated object or array.
func RepairJSON(arguments json.RawMessage) (json.RawMessage, bool) {
	...
	candidates := [][]byte{}
	if repaired, ok := repairQuoteBeforeTrailingClosers(trimmed); ok {
		candidates = append(candidates, repaired)
		if closed, ok := repairAppendClosers(repaired); ok {
			candidates = append(candidates, closed)
		}
	}
	if repaired, ok := repairAppendClosers(trimmed); ok {
		candidates = append(candidates, repaired)
	}
...
func repairQuoteBeforeTrailingClosers(arguments []byte) ([]byte, bool) {
	state := scanJSONState(arguments)
...
func repairAppendClosers(arguments []byte) ([]byte, bool) {
	state := scanJSONState(arguments)
...
func scanJSONState(arguments []byte) jsonState {
	var state jsonState
