...
func TestWebSearchSelfRegisters(t *testing.T) {
	found := false
	for _, tool := range Registered() {
		if tool.Metadata().Name == "web_search" {
			found = true
		}
	}
	if !found {
		t.Fatal("web_search was not registered via init")
	}
}

func TestLLMSchemaIncludesWebSearch(t *testing.T) {
	schema := LLMSchema()
	found := false
	for _, tool := range schema {
		if tool.Function.Name == "web_search" {
			...
			if tool.Function.Description == "" {
				t.Fatal("web_search missing description in LLM schema")
			}
			if tool.Function.Parameters == nil {
				t.Fatal("web_search missing parameters in LLM schema")
			}
		}
	}
	if !found {
		t.Fatal("web_search missing from LLMSchema")
	}
}
