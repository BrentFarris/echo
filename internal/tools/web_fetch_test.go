...
func TestWebFetchSelfRegisters(t *testing.T) {
	found := false
	for _, tool := range Registered() {
		if tool.Metadata().Name == "web_fetch" {
			found = true
		}
	}
	if !found {
		t.Fatal("web_fetch was not registered via init")
	}
}

func TestLLMSchemaIncludesWebFetch(t *testing.T) {
	schema := LLMSchema()
	found := false
	for _, tool := range schema {
		if tool.Function.Name == "web_fetch" {
			...
			if tool.Function.Description == "" {
				t.Fatal("web_fetch missing description in LLM schema")
			}
			if tool.Function.Parameters == nil {
				t.Fatal("web_fetch missing parameters in LLM schema")
			}
		}
	}
	if !found {
		t.Fatal("web_fetch missing from LLMSchema")
	}
}
