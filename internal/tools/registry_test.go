...
func TestReadOnlyLLMSchemaIncludesOnlyInspectionTools(t *testing.T) {
	...
	for _, name := range []string{"filesystem_list", "filesystem_read_image", "filesystem_read_text", "filesystem_search_text", "filesystem_search_workspace", "filesystem_stat", "git_inspect", "lsp_query", "web_search", "workspace_context", "workspace_skill_read", "workspace_skill_search"} {
		if !names[name] {
