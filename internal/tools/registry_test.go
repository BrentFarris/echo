...
func TestDefaultRegistryIncludesFilesystemTools(t *testing.T) {
	...
	for _, name := range []string{"filesystem_create_text", "filesystem_delete_file", "filesystem_edit_text", "filesystem_list", "filesystem_read_image", "filesystem_read_text", "filesystem_search_text", "filesystem_stat", "lsp_query", "shell_command", "web_search"} {
		if !names[name] {
...
func TestReadOnlyLLMSchemaIncludesOnlyInspectionTools(t *testing.T) {
	...
	for _, name := range []string{"filesystem_list", "filesystem_read_image", "filesystem_read_text", "filesystem_search_text", "filesystem_stat", "lsp_query", "web_search"} {
		if !names[name] {
	...
	if len(names) != 7 {
		t.Fatalf("expected exactly seven read-only tools, got %#v", names)
	}
}
