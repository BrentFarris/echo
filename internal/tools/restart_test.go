...
func TestRestartToolRequiresEchoWorkspace(t *testing.T) {
	...
	if result.Success {
		t.Fatalf("expected failure when workspace lacks an Echo go.mod, got success")
	}
...
func TestRestartToolRejectsNonEchoGoMod(t *testing.T) {
	tmpDir := t.TempDir()
	gomod := filepath.Join(tmpDir, "go.mod")
	if err := os.WriteFile(gomod, []byte("module example.com/other\n"), 0644); err != nil {
	...
	result := Execute(ctx, "restart", json.RawMessage("{}"))
	if result.Success {
		t.Fatalf("expected failure for a non-Echo go.mod, got success")
	}
	if result.Error == nil || result.Error.Code != "not_echo_workspace" {
		t.Fatalf("expected error code 'not_echo_workspace', got %v", result.Error)
	}
...
func TestRestartToolLaunchesDetachedProcess(t *testing.T) {
	...
	// Create a go.mod declaring the Echo module so the workspace check passes.
	gomod := filepath.Join(tmpDir, "go.mod")
	if err := os.WriteFile(gomod, []byte("module github.com/brent/echo\n"), 0644); err != nil {
		t.Fatalf("failed to create go.mod: %v", err)
	}
