...
func parseStreamWithActivity(ctx context.Context, reader io.Reader, events chan<- StreamEvent, usageOut **Usage, onActivity func()) {
	...
	for scanner.Scan() {
		...
		line := scanner.Text()
		if line == "" {
			if flushStreamData(ctx, events, dataLines, &completed, usageOut) {
				return
			}
			dataLines = nil
			continue
	...
	if len(dataLines) > 0 && flushStreamData(ctx, events, dataLines, &completed, usageOut) {
		return
	}
	if ctx.Err() != nil {
		emitCanceledLogged(events)
...
func flushStreamData(ctx context.Context, events chan<- StreamEvent, dataLines []string, completed *bool, usageOut **Usage) bool {
	if len(dataLines) == 0 {
		return false
	}
	...
	if data == "[DONE]" {
		if !*completed {
			emitLogged(ctx, events, StreamEvent{Type: EventComplete})
			*completed = true
		}
		return true
	}
	...
	if err := json.Unmarshal([]byte(data), &chunk); err != nil {
		emitLogged(ctx, events, StreamEvent{Type: EventError, Error: fmt.Sprintf("decode stream chunk: %v", err)})
		return true
	}
	...
	for _, choice := range chunk.Choices {
		...
		for _, toolCall := range choice.Delta.ToolCalls {
			copy := toolCall
			emitLogged(ctx, events, StreamEvent{Type: EventToolCall, ToolCall: &copy, Raw: json.RawMessage(data)})
		}
		if choice.FinishReason != nil && !*completed {
			emitLogged(ctx, events, StreamEvent{Type: EventComplete, FinishReason: *choice.FinishReason, Raw: json.RawMessage(data)})
			*completed = true
		}
	}
	if chunk.Usage != nil {
		usage := *chunk.Usage
		emitLogged(ctx, events, StreamEvent{Type: EventUsage, Usage: &usage, Raw: json.RawMessage(data)})
		if usageOut != nil {
			*usageOut = &usage
		}
	}
...
func emitLogged(ctx context.Context, events chan<- StreamEvent, event StreamEvent) bool {
	select {
	case <-ctx.Done():
		return false
	case events <- event:
...
func emitCanceledLogged(events chan<- StreamEvent) {
	select {
