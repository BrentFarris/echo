...
import type {
  GitActionRequest, GitActionResult, GitChange, GitCommitDetail, GitHistory, GitMetadata,
  GitOperationEvent, GitRepository, GitStatus,
} from "./gitTypes";
...
type GitViewCallbacks = {
  ...
  openDiff(repository: GitRepository, change: GitChange | { path: string; oldPath?: string; ref?: FileRef }, scope: "staged" | "unstaged" | "commit" | "stash", ref: string | undefined, pin: boolean): Promise<void>;
...
export class GitView {
  ...
  private readonly callbacks: GitViewCallbacks;
  private repositories: GitRepository[] = [];
  private statuses = new Map<string, GitStatus>();
  private selections = new Map<string, SelectionState>();
  private drafts = new Map<string, string>();
  private expandedRepositories = new Set<string>();
  private expandedGroups = new Set<string>();
  private busyRepositories = new Map<string, GitOperationEvent>();
  private metadata = new Map<string, GitMetadata>();
  private history = new Map<string, GitHistory>();
  ...
  constructor(host: HTMLElement, workspaceId: string, signal: AbortSignal, callbacks: GitViewCallbacks) {
    this.host = host;
  ...
  private installSocket(): void {
    ...
    const unsubscribeStatus = onSocket("git_status", (data: object) => {
    ...
    const unsubscribeOperation = onSocket("git_operation", (data: object) => {
      const event = data as { workspaceId: string; operation?: GitOperationEvent };
    ...
    const unsubscribeResync = onSocket("git_resync_required", (data: object) => {
  ...
  private async reloadRepositories(): Promise<void> {
    try {
      ...
      this.searchParents = response.searchParentGitRepositories;
      if (this.expandedRepositories.size === 0) this.repositories.forEach((repository) => this.expandedRepositories.add(repository.id));
      this.loading = false;
      this.render();
      await Promise.allSettled(this.repositories.map((repository) => this.refreshStatus(repository.id)));
    } catch (error) {
      this.loading = false;
  ...
  private async refreshStatus(repositoryId: string): Promise<void> {
    try {
      this.acceptStatus(await gitAPI.loadStatus(this.workspaceId, repositoryId));
    } catch (error) {
  ...
  private changeKey(change: GitChange): string {
  ...
  private renderRepositorySelector(repository: GitRepository): string {
    const status = this.statuses.get(repository.id);
    return `<button type="button" class="git-repository-selector" data-git-repository-toggle="${escapeHTML(repository.id)}" title="${escapeHTML(repository.label)}">
      ...
      <b>${status?.totalChangeCount || 0}</b>
    </button>`;
  }

  private renderNoRepositories(): string {
    return `<div class="git-empty"><span class="codicon codicon-source-control"></span><p>No Git repositories were found in this workspace.</p><button type="button" class="code-primary-button" data-git-global="initialize">Initialize Repository</button><button type="button" data-git-global="clone">Clone Repository</button></div>`;
  }

  private renderRepository(repository: GitRepository): string {
    ...
    return `<article class="git-repository" data-git-repository="${escapeHTML(repository.id)}">
      <header class="git-repository-header">
        <button type="button" data-git-repository-toggle="${escapeHTML(repository.id)}" aria-expanded="${expanded}">
          ...
          <strong>${escapeHTML(repository.label)}</strong>${repository.parent ? `<em title="Parent repository">parent</em>` : ""}
        </button>
      ...
      ${expanded ? this.renderRepositoryBody(repository, status) : ""}
    </article>`;
  }

  private renderRepositoryBody(repository: GitRepository, status: GitStatus | undefined): string {
    ...
    return `<div class="git-repository-body">
      ...
      ${this.renderGroup(repository, "staged", "Staged Changes", status.staged, "unstage_all")}
      ${this.renderGroup(repository, "unstaged", "Changes", status.unstaged, "stage_all")}
      ${status.totalChangeCount === 0 ? `<div class="git-empty compact"><span class="codicon codicon-check-all"></span>No changes</div>` : ""}
      ${this.renderHistory(repository)}
    </div>`;
  }

  private renderGroup(repository: GitRepository, scope: "conflict" | "staged" | "unstaged", label: string, changes: GitChange[], action: string): string {
    ...
    const icon = scope === "staged" ? "remove" : "add";
    const title = scope === "staged" ? "Unstage All Changes" : "Stage All Changes";
    const busy = this.busyRepositories.has(repository.id);
    return `<section class="git-change-group" data-git-group="${scope}">
      <header>
        <button type="button" data-git-group-toggle="${scope}" aria-expanded="${expanded}"><span class="codicon codicon-chevron-${expanded ? "down" : "right"}"></span><span>${escapeHTML(label)}</span><b>${changes.length}</b></button>
        <button type="button" title="${title}" aria-label="${title}" data-git-group-action="${action}" ${busy ? "disabled" : ""}><span class="codicon codicon-${icon}"></span></button>
      </header>
      ${expanded ? `<div class="git-change-list" data-git-scroll-key="${escapeHTML(key)}" data-git-list-key="${escapeHTML(key)}" data-git-list-repository="${escapeHTML(repository.id)}" data-git-list-scope="${scope}" role="listbox" aria-multiselectable="true"><div class="git-change-canvas"></div></div>` : ""}
    </section>`;
  }

  private renderHistory(repository: GitRepository): string {
    ...
    return `<section class="git-history">
      <header><button type="button" data-git-history-toggle aria-expanded="${expanded}"><span class="codicon codicon-chevron-${expanded ? "down" : "right"}"></span><span class="codicon codicon-git-commit"></span><strong>GRAPH</strong></button></header>
  ...
  private renderReview(repository: GitRepository, review: Review): string {
  ...
  private mountVirtualLists(): void {
    this.host.querySelectorAll<HTMLElement>("[data-git-list-key]").forEach((list) => {
      ...
      const scope = list.dataset.gitListScope as "conflict" | "staged" | "unstaged";
      const status = this.statuses.get(repositoryId);
      const changes = scope === "conflict" ? status?.conflicts : scope === "staged" ? status?.staged : status?.unstaged;
      if (!changes) return;
  ...
  private renderHistoryRow(commit: GitHistory["commits"][number], index: number): string {
  ...
  private renderChangeRow(repositoryId: string, scope: string, change: GitChange, index: number): string {
    ...
    return `<div class="git-change-row ${selected ? "is-selected" : ""}" style="transform:translateY(${index * rowHeight}px)" role="option" aria-selected="${selected}" tabindex="${selected ? 0 : -1}" data-git-change-index="${index}">
      ...
      <div class="git-change-actions">
        ${scope === "unstaged" ? `<button type="button" title="Open File" aria-label="Open File" data-git-file-action="open"><span class="codicon codicon-go-to-file"></span></button><button type="button" title="Revert Changes" aria-label="Revert Changes" data-git-file-action="discard" ${busy ? "disabled" : ""}><span class="codicon codicon-discard"></span></button>` : ""}
        <button type="button" title="${scope === "staged" ? "Unstage Changes" : "Stage Changes"}" aria-label="${scope === "staged" ? "Unstage Changes" : "Stage Changes"}" data-git-file-action="${scope === "staged" ? "unstage" : "stage"}" ${busy ? "disabled" : ""}><span class="codicon codicon-${scope === "staged" ? "remove" : "add"}"></span></button>
      </div>
  ...
  private async handleClick(event: MouseEvent): Promise<void> {
    ...
    if (reviewFile) {
      ...
      if (review && file) await this.callbacks.openDiff(repository, file, review.kind, review.ref, true);
      return;
    }
    ...
    if (!event.ctrlKey && !event.metaKey && !event.shiftKey) await this.callbacks.openDiff(repository, context.change, context.scope === "staged" ? "staged" : "unstaged", undefined, false);
    this.render();
  }

  private changeContext(row: HTMLElement): { scope: "conflict" | "staged" | "unstaged"; changes: GitChange[]; change: GitChange; index: number } | null {
  ...
  private async openChange(row: HTMLElement, pin: boolean): Promise<void> {
    ...
    const repository = this.repositories.find((candidate) => candidate.id === repositoryId);
    if (context && repository) await this.callbacks.openDiff(repository, context.change, context.scope === "staged" ? "staged" : "unstaged", undefined, pin);
  }

  private async handleFileAction(repository: GitRepository, scope: string, changes: GitChange[], index: number, action: string): Promise<void> {
    const change = changes[index];
    if (!change) return;
    if (action === "open") {
  ...
  private async run(repository: GitRepository, request: GitActionRequest): Promise<void> {
    const operation: GitOperationEvent = { workspaceId: this.workspaceId, repositoryId: repository.id, requestId: request.requestId, action: request.action, state: "running" };
    this.busyRepositories.set(repository.id, operation);
    this.render();
    try {
      const result = await gitAPI.runAction(this.workspaceId, repository.id, request);
  ...
  private applyPrediction(repositoryId: string, request: GitActionRequest, result: GitActionResult): void {
    const current = this.statuses.get(repositoryId);
    if (current) this.statuses.set(repositoryId, predictGitStatus(current, request, result));
  }
  ...
  private showGlobalMenu(x: number, y: number): void {
    showContextMenu(x, y, [
      ...
      { label: "Initialize Repository…", icon: "repo-create", run: () => this.initialize() },
      { label: "Clone Repository…", icon: "repo-clone", run: () => this.clone() },
  ...
  private showRepositoryMenu(repository: GitRepository, x: number, y: number): void {
    showContextMenu(x, y, [
      { label: "Refresh", icon: "refresh", run: () => this.refreshStatus(repository.id) },
      { label: "Commit…", icon: "git-commit", separatorBefore: true, run: () => this.showCommitMenu(repository, x, y) },
      { label: "Changes…", icon: "list-selection", run: () => this.showChangesMenu(repository, x, y) },
      { label: "Pull, Push…", icon: "sync", run: () => this.showNetworkMenu(repository, x, y) },
      { label: "Branch…", icon: "git-branch", run: () => void this.showBranchMenu(repository, x, y) },
      { label: "Remote…", icon: "remote", run: () => void this.showRemoteMenu(repository, x, y) },
      { label: "Stash…", icon: "archive", run: () => void this.showStashMenu(repository, x, y) },
      { label: "Tags…", icon: "tag", run: () => void this.showTagMenu(repository, x, y) },
      { label: this.historyExpanded.has(repository.id) ? "Hide Commit Graph" : "Show Commit Graph", icon: "git-commit", separatorBefore: true, run: () => {
        if (this.historyExpanded.has(repository.id)) this.historyExpanded.delete(repository.id);
  ...
  private showCommitMenu(repository: GitRepository, x: number, y: number): void {
    const status = this.statuses.get(repository.id);
    const disabled = this.busyRepositories.has(repository.id);
    showContextMenu(x, y, [
      { label: "Commit Staged", icon: "git-commit", disabled: disabled || !status?.staged.length, run: () => this.commit(repository.id, "commit_staged") },
      { label: "Commit All", icon: "git-commit", disabled: disabled || !status?.unstaged.length, run: () => this.commit(repository.id, "commit_all") },
  ...
  private showChangesMenu(repository: GitRepository, x: number, y: number): void {
    const status = this.statuses.get(repository.id);
    showContextMenu(x, y, [
  ...
  private showNetworkMenu(repository: GitRepository, x: number, y: number): void {
    showContextMenu(x, y, [
      { label: "Sync", icon: "sync", run: () => this.runSimple(repository, "sync") },
      { label: "Pull", icon: "cloud-download", separatorBefore: true, run: () => this.runSimple(repository, "pull") },
      ...
      { label: "Push", icon: "cloud-upload", separatorBefore: true, run: () => this.runSimple(repository, "push") },
  ...
  private async showBranchMenu(repository: GitRepository, x: number, y: number): Promise<void> {
  ...
  private async showRemoteMenu(repository: GitRepository, x: number, y: number): Promise<void> {
    const metadata = await this.ensureMetadata(repository.id);
    if (!metadata) return;
    showContextMenu(x, y, [
  ...
  private async showStashMenu(repository: GitRepository, x: number, y: number): Promise<void> {
    const metadata = await this.ensureMetadata(repository.id);
    if (!metadata) return;
  ...
  private async showTagMenu(repository: GitRepository, x: number, y: number): Promise<void> {
    const metadata = await this.ensureMetadata(repository.id);
    if (!metadata) return;
    showContextMenu(x, y, [
  ...
  private async ensureMetadata(repositoryId: string): Promise<GitMetadata | null> {
    const cached = this.metadata.get(repositoryId);
    if (cached) return cached;
    try {
      const metadata = await gitAPI.loadMetadata(this.workspaceId, repositoryId);
  ...
  private async loadHistory(repositoryId: string): Promise<void> {
    try { this.history.set(repositoryId, await gitAPI.loadHistory(this.workspaceId, repositoryId)); this.render(); }
  ...
  private async loadMoreHistory(repositoryId: string): Promise<void> {
    ...
    try {
      const next = await gitAPI.loadHistory(this.workspaceId, repositoryId, current.nextOffset || current.commits.length);
  ...
  private async loadReview(repositoryId: string, ref: string, kind: "commit" | "stash"): Promise<void> {
    try {
      const detail = await gitAPI.loadCommitDetail(this.workspaceId, repositoryId, ref, kind);
  ...
  private async initialize(): Promise<void> {
    ...
    if (!root) return;
    try { await gitAPI.initializeRepository(this.workspaceId, root.id); await this.reloadRepositories(); }
  ...
  private async clone(): Promise<void> {
    ...
    try { await gitAPI.cloneRepository(this.workspaceId, url, root.id, destination); await this.reloadRepositories(); }
    catch (error) { toast(error instanceof Error ? error.message : String(error), { sticky: true }); }
  }

  private async toggleParentSearch(): Promise<void> {
    try { await gitAPI.setParentRepositorySearch(this.workspaceId, !this.searchParents); this.searchParents = !this.searchParents; await this.reloadRepositories(); }
    catch (error) { toast(error instanceof Error ? error.message : String(error)); }
  }
}
