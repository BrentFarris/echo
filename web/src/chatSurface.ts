...
import {
  composerText, restoreComposer, snapshotComposer,
...
export function mountChatSurface(host: HTMLElement, options: ChatSurfaceOptions): MountedChatSurface {
  ...
  const mentions = installMentionPicker(input, inputWrap, { workspaceId: options.workspaceId, signal, onActivateReference: options.onActivateReference });
  const clearMention = mentions.clear;
  ...
  input.addEventListener("input", saveDraft, { signal });
  input.addEventListener("keydown", (event) => {
    const keyboard = event as KeyboardEvent;
    if (mentions.handleKeydown(keyboard)) return;
